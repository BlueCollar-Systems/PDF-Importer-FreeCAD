#!/usr/bin/env python3
"""Build a deterministic, privacy-checked structural-steel shape archive."""
...
FORBIDDEN_ARTIFACT_SUFFIXES = {".pdf", ".skp", ".fcstd", ".blend"}
MACHINE_PATH_MARKERS = (b"c:\\users\\", b"c:/users/")
TEXT_SUFFIXES = {".csv", ".dxf", ".md", ".py", ".rb", ".txt"}
...
def _archive_content(path: Path) -> bytes:
...
def build(source_dir: Path, out_dir: Path, tag: str) -> BuildArtifacts:
    ...
    versioned = destination / f"Structural-Steel-DXF-DWG-Shapes-{version}.zip"
    latest = destination / "Structural-Steel-DXF-DWG-Shapes-latest.zip"
    ...
    with zipfile.ZipFile(
        ...
        for path in files:
            ...
            archive.writestr(info, _archive_content(path), compresslevel=9)
