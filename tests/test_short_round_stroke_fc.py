...
from PDFStrokeFootprint import capsule_edges, native_face, short_round_stroke, unclipped_capsules, bind_similarity_strokes
...
def test_native_builder_maps_every_source_control_point_and_uses_true_arcs():
    class Part:
        class Edge:
            def __init__(self, kind, points):
                self.kind, self.points = kind, points
            def toShape(self):
                return self.kind, self.points
        @classmethod
        def LineSegment(cls, *points):
            return cls.Edge("line", points)
        @classmethod
        def Arc(cls, *points):
            return cls.Edge("arc", points)
        Wire = staticmethod(tuple)
        Face = staticmethod(lambda wire: wire)
    cap = short_round_stroke(stroke())
    mapper = lambda point: (point[1] * 2, -point[0] * 2, 0)
    result = native_face(cap, mapper, Part)
    assert result == tuple((kind, tuple(mapper(p) for p in points))
                           for kind, *points in capsule_edges(cap))
